...
export const STORE_CONFIG = {
  ...
  // WhatsApp raw number without leading '+' or spaces (e.g., 923001234567)
  ...
  phoneNumber: import.meta.env.VITE_PHONE_NUMBER || '+923001234567',
  ...
  getWhatsAppUrl: (message: string = 'Salam Jadugar Accessories, I have an inquiry') => {
    const number = import.meta.env.VITE_WHATSAPP_NUMBER || '923001234567';
    return `https://wa.me/${number}?text=${encodeURIComponent(message)}`;
  },

  // Helper function for phone call link
  getTelUrl: () => {
    const number = import.meta.env.VITE_PHONE_NUMBER || '+923001234567';
    return `tel:${number}`;
  }
};
